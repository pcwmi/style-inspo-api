...
class StyleGenerationEngine:
    ...
    def __init__(
        ...
        Args:
            ...
            prompt_version: Prompt template version (default: baseline_v1)
        """
        self.model = model
        ...
        # Get prompt template
        self.prompt_template = PromptLibrary.get_prompt(prompt_version)
    ...
    def create_style_prompt(self,
        ...
        Now uses prompt template system for A/B testing and extensibility.
        """
        ...
        return self.prompt_template.build(context)
    ...
    def _find_item_by_name(self, item_name: str, all_items: List[Dict]) -> Optional[Dict]:
        """Find a wardrobe item by name (fuzzy matching with HTML entity normalization)"""
        item_name_normalized = self._normalize_item_name(item_name)

        # First try exact match
        for item in all_items:
            wardrobe_name = item.get('styling_details', {}).get('name', '')
            if self._normalize_item_name(wardrobe_name) == item_name_normalized:
                return item

        # Then try partial match
        for item in all_items:
            wardrobe_name = item.get('styling_details', {}).get('name', '')
            wardrobe_name_normalized = self._normalize_item_name(wardrobe_name)
