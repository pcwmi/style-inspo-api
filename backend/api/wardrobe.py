...
@router.post("/wardrobe/{user_id}/fix-orientation")
async def fix_orientation(user_id: str):
    """
    Batch fix EXIF orientation for all wardrobe items.
    Removes EXIF orientation tags to prevent browser double-rotation.
    Throwaway endpoint - will be deleted after one-time use.
    """
    try:
        from io import BytesIO
        ...
        for item in items:
            try:
                ...
                # Load image
                image_data = manager.storage.load_file(image_path)
                if not image_data:
                    continue
                ...
                if 'exif' in img.info:
                    ...
                    manager.storage.save_image(img, filename)
                    fixed_count += 1

            except Exception as e:
                logger.error(f"Error fixing orientation for item {item.get('id')}: {e}")
                continue
        ...
        logger.error(f"Error in fix_orientation for {user_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
