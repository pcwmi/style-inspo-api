...
export function OutfitCard({ outfit, user, index, allowSave = true, allowDislike = true }: OutfitCardProps) {
    ...
    return (
        <div className="bg-white border border-[rgba(26,22,20,0.12)] rounded-lg p-4 md:p-6 mb-4 md:mb-6 shadow-sm">
            ...
            {/* Why it works - hide if empty */}
            {outfit.why_it_works && outfit.why_it_works.trim() !== '' && (
                <div className="mb-3 md:mb-4">
            ...
            {/* Agent reasoning - collapsible "Why this outfit" */}
            {outfit.agent_reasoning && outfit.agent_reasoning.length > 0 && (
                <div className="mb-3 md:mb-4">
                    <button
                    ...
                    >
                        <svg
                            className={`w-3.5 h-3.5 transition-transform ${reasoningExpanded ? 'rotate-90' : ''}`}
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
                    ...
                    {reasoningExpanded && (
                        <div className="mt-2 p-3 bg-bone rounded-lg border border-sand text-sm space-y-2">
                            {outfit.agent_reasoning.map((entry: any, idx: number) => (
                                <div key={idx}>
                                    <span className="font-medium text-muted text-xs uppercase tracking-wide">
                                        {entry.tool.replace(/_/g, ' ')}
                                    </span>
                                    <p className="text-ink leading-relaxed mt-0.5">{entry.reasoning}</p>
                                </div>
                            ))}
                        </div>
                    )}
